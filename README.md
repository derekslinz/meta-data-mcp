...
- **Generator + tooling (v2.0 Phase 6):** `response_shape` field in the spec schema drives shape binding at generate time; `test_ui_bundle_sizes.py` enforces the 100 KB warn / 1 MB error budget in CI; `tests/smoke/test_ui_bundles.py` loads each bundle headlessly via Playwright; `make pr-check N=<pr>` wraps the merge-gate checklist.
