...
> A single MCP server that transparently routes user requests to 77 open-data sources.

`meta-data-mcp` is one MCP server — not many. Under the hood it bundles 77 *plugins*, each wrapping a different open-data API. The plugins are an implementation detail; from your LLM's perspective there is one server and one place to ask "where can I find data about X?"
...
2. **Plugin tools** — ~330 tools coming from the 77 bundled plugins. In the default discovery-only mode they are activated per provider at runtime (or preloaded via `META_DATA_MCP_PRELOAD`). The LLM picks one after consulting the meta tools.
...
- Expand provider coverage beyond the current 77.
